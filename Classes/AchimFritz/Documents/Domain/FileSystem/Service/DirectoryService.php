...
class DirectoryService {
	...
	public function getCountOfFilesByExtension($path, $extension) {
		try {
			...
			throw new \AchimFritz\Documents\Domain\FileSystem\Service\Exception('cannot create DirectoryIterator with path ' . $path, 1419357137);
		}
		$cnt = 0;
	...
	public function getSplFileInfosInDirectory($path, $extension) {
		...
		try {
			...
			throw new \AchimFritz\Documents\Domain\FileSystem\Service\Exception('cannot create DirectoryIterator with path ' . $path, 1419357135);
		}
		foreach ($directoryIterator AS $fileInfo) {
			if ($extension !== '') {
				if ($fileInfo->getExtension() === $extension) {
	...
	public function getDirectoriesInDirectory($path) {
		...
		try {
			...
			throw new \AchimFritz\Documents\Domain\FileSystem\Service\Exception('cannot create DirectoryIterator with path ' . $path, 1419357136);
		}
		foreach ($directoryIterator AS $fileInfo) {
	...
	public function getDirectoriesInDirectoryRecursive($path, $depth = 0) {
		...
		try {
			...
			throw new \AchimFritz\Documents\Domain\FileSystem\Service\Exception('cannot create DirectoryIterator with path ' . $path, 1419357156);
		}
